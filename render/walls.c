...
void	init_wall_values(t_wall *w)
{
	...
	w->t->texture = mlx_load_png("textures/stone_bricks.png");
	w->t->arr = (uint32_t *)w->t->texture->pixels;
	w->distance_to_projection = (WIDTH * BLOCK_W / 2)
		/ tan(FOV / 2 * (M_PI / 180));
}

void	update_wall_values(t_wall *w, t_ray *rays, t_map *m)
{
...
void	render_all_walls(t_map *m, t_ray *rays, t_wall *w)
{
	int		x;
